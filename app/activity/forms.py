...
class ActivityForm(forms.ModelForm):
    ...
    class Meta:
        model = Activity
        fields = ['file_input', 'service_type_id', 'board_id', 'standard_id', 'subject_id', 'chapter_id', 'generation_type', 'activity_time_in_mins', 'language', 'activity_type']
    ...
    def save(self, commit=True, **kwargs):
        ...
        print("clean data in creare", cleaned_data)
        activity_attrs = {
            'board_id': cleaned_data.get('board_id'),
            ...
            'activity_time_in_mins': cleaned_data.get('activity_time_in_mins'),
        ...
        user_prompt = 'Generate a classroom activity from this document'
        if ans := ai.generate_questions(user_prompt, cleaned_data, activity_attrs):
            print("answer-----------", ans)
            activity_attrs['description'] = ans
            activity = Activity.objects.create(**activity_attrs)
            print("cretd ", activity)
            return activity